...
2. 利用友元和模板的特性通过一个工具类（class EveryMsg;）将数据排列在连续的buffer中，或从buffer中依次取出。
...
3. 如果我们针对每一种数据结构都去实现数据排列规则和算法那么程序代码量呈线形增长，既不优雅也很繁琐(google的protobuf工具就是用的这种方式)。但其实我们可以通过宏函数入侵的方式嵌入应用层代码，由应用层去指定排列顺序，同时通过友元和模板的机制实现排列数据的规则和算法处理，并将其处理过程提炼出一个泛型的工具类完成序列化与反序列化框架。
