...
#ifndef USE_STL_SAFETY
//#define USE_STL_SAFETY     //启用STL容器安全
#endif // !USE_STL_SAFETY
...
#define OUTPUT_DATA(_TYPE)                 \
...
#else
	//序列化输入; 除了基本数据类型使用模板(template<class T>)以外，其他的需要实现个性化序列化;
	/*
	EveryMsg& operator << (char& value);
	EveryMsg& operator << (short& value);
	EveryMsg& operator << (int& value);
	EveryMsg& operator << (long& value);
	EveryMsg& operator << (float& value);
	EveryMsg& operator << (double& value);
	.
	.
	.
	*/
	...
	//std::string个性化输入;
	inline EveryMsg& operator << (const string& value)
	{
		//+1 加上字符串的最后一个0
	...
	//std::vector<int>个性化输入;
	inline EveryMsg& operator << (const vectorint& value)
	{
	...
	//std::vector<string>个性化输入;
	inline EveryMsg& operator << (const vectorstring& value)
	{
		unsigned int nSize = value.size();
		*this << nSize;
		for (auto& item : value)
...
#else
	//序列化输出; 除了基本数据类型使用模板(template<class T>)以外，其他的需要实现个性化序列化;
	/*
	EveryMsg& operator >> (char& value);
	EveryMsg& operator >> (short& value);
	EveryMsg& operator >> (int& value);
	...
	*/
	...
	//std::string个性化输出;
	inline EveryMsg& operator >> (string& value)
	{
	...
	//std::vector<int>个性化输出;
	inline EveryMsg& operator >> (vectorint& value)
	{
		value.clear();
	...
	//std::vector<string>个性化输出;
	inline EveryMsg& operator >> (vectorstring& value)
	{
		value.clear();
		unsigned int nSize = 0;
		*this >> nSize;
